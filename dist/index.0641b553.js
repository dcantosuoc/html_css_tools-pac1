...
function fntExecuteSlider(side) {
    ...
    for(var i = 0; i < elements.length; i++)if (elements[i].classList.contains("slider_activa") == true) {
        curElement = i;
        break;
    }
    if (side == "prev" || side == "next") {
        if (side == "prev") nextElement = curElement == 0 ? elements.length - 1 : curElement - 1;
        else nextElement = curElement == elements.length - 1 ? 0 : curElement + 1;
    } else {
        nextElement = side;
...
if (document.querySelector("#container-slider")) runSlider();
